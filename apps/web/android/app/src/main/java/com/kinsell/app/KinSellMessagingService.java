...
public class KinSellMessagingService extends FirebaseMessagingService {
    ...
    private void showIncomingCallNotification(RemoteMessage msg, String title, String body) {
        ...
        String expiresAtRaw = data.get("expiresAt");
        ...
        // ── Étape 3 : verrouillage des push d'appel ──
        // Sans callId + expiresAt non expiré, on ne réveille pas l'utilisateur.
        // Cela neutralise les vieux push résiduels après reboot/réinstall qui
        // ressuscitaient un appel zombie.
        long expiresAt = 0L;
        try { expiresAt = expiresAtRaw != null ? Long.parseLong(expiresAtRaw) : 0L; }
        catch (NumberFormatException ignored) { expiresAt = 0L; }
        long nowMs = System.currentTimeMillis();
        if (callId.isEmpty() || expiresAt <= 0L || expiresAt <= nowMs) {
            // Drop silencieux — pas de notif, pas de persistance.
            return;
        }
        ...
        try {
            ...
            prefs.edit()
                .putString("pending_incoming_call",
                    ...
                    "|" + callId + "|" + expiresAt)
                .putLong("pending_incoming_call_ts", nowMs)
                .apply();
        ...
        acceptIntent.putExtra("expiresAt", expiresAt);
