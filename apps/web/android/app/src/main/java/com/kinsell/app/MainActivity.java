...
public class MainActivity extends BridgeActivity {
    ...
    private final BroadcastReceiver callRejectReceiver = new BroadcastReceiver() {
        ...
        public void onReceive(Context context, Intent intent) {
            ...
            // Étape 3 : sans callId/expiresAt valides, on dispatch quand même
            // l'event UI (l'app peut clore son UI incoming locale) mais sans
            // callId, le client ne tentera pas de reject serveur.
            dispatchCallRejectToWebView(conversationId, callerId, callId, expiresAt);
        }
    };
    ...
    private void flushPendingCallReject() {
        try {
            ...
            if (pendingReject != null && !pendingReject.isEmpty()) {
                ...
                // Format étape 3 : conversationId|callerId|callType|callId|expiresAt
                String[] parts = pendingReject.split("\\|", -1);
                if (parts.length < 2) return;
                String conversationId = parts[0];
                String callerId = parts[1];
                String callId = parts.length > 3 ? parts[3] : "";
                long expiresAt = 0L;
                if (parts.length > 4) {
                    try { expiresAt = Long.parseLong(parts[4]); }
                    catch (NumberFormatException ignored) { expiresAt = 0L; }
                }
                // Drop si callId/expiresAt absents ou expirés : un reject sans
                // référence serveur valide ne doit pas être rejoué.
                if (callId.isEmpty() || expiresAt <= 0L || expiresAt <= now) return;
                dispatchCallRejectToWebView(conversationId, callerId, callId, expiresAt);
            }
        } catch (Exception ignored) {}
    }
     ...
     * Étape 3 : exige callId + expiresAt non expiré, sinon drop silencieux.
     */
    private void flushPendingIncomingCall() {
        try {
            ...
            // Expiration locale : si l'appel date de plus de 35s, on n'essaie même pas.
            if (now - ts > 35_000L) return;

            String[] parts = pendingCall.split("\\|", -1);
            if (parts.length < 3) return;
            String conversationId = parts[0];
            String callerId = parts[1];
            String callType = parts[2];
            String callerName = parts.length > 3 ? parts[3] : "";
            String callId = parts.length > 4 ? parts[4] : "";
            long expiresAt = 0L;
            if (parts.length > 5) {
                try { expiresAt = Long.parseLong(parts[5]); }
                catch (NumberFormatException ignored) { expiresAt = 0L; }
            }

            // Étape 3 : verrou strict sur callId + expiresAt non expiré.
            if (callId.isEmpty() || expiresAt <= 0L || expiresAt <= now) return;
            ...
            if (webView != null) {
                ...
                final String cidServer = sanitizeForJs(callId);
                final long exp = expiresAt;
     ...
     * Étape 3 : transmet callId+expiresAt pour que le client puisse
     * émettre call:reject avec une référence serveur valide.
     */
    private void dispatchCallRejectToWebView(String conversationId, String callerId, String callId, long expiresAt) {
        disableKeepScreenOn();
        try {
            WebView webView = getBridge().getWebView();
            if (webView != null) {
