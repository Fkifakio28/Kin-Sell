...
import { setEarpiece, setSpeaker, resetAudioRoute, getAudioRoutes, setAudioRoute as nativeSetAudioRoute, addAudioRouteListener, type AudioRoute as NativeAudioRoute } from "../utils/audio-route";
...
export type ActiveCall = {
  ...
  /** Identifiant unique de l'appel (id du CallLog serveur). Manque
   *  brièvement côté caller entre l'émission de call:initiate et la réception
   *  de call:initiated, et côté receiver si l'injection externe (URL/push)
   *  ne le fournit pas — sera traité en étape 3. */
  callId?: string;
  /** Timestamp d'expiration côté serveur (ms epoch). Pareil : peut être
   *  absent dans les flux d'injection externes en attendant l'étape 3. */
  expiresAt?: number;
};
...
export function useAudioCallState() {
  ...
  // Étape 5 — route audio active + routes disponibles (earpiece/speaker/bluetooth)
  const [audioRoute, setAudioRouteState] = useState<NativeAudioRoute>("earpiece");
  const [availableAudioRoutes, setAvailableAudioRoutes] = useState<NativeAudioRoute[]>(["earpiece", "speaker"]);
  ...
  const cleanup = useCallback(() => {
    ...
    setAvailableAudioRoutes(["earpiece", "speaker"]);
  ...
  const armRingingWatchdog = useCallback(() => {
    ...
    ringingWatchdogRef.current = setTimeout(() => {
      ...
      if (c.status === "outgoing_ringing") {
        ...
        try {
          if (c.callId) emit("call:end", { callId: c.callId });
        } catch { /* ignore */ }
  ...
  const startCall = useCallback(async (conversationId: string, remoteUserId: string) => {
    ...
    try {
      ...
      // P0 #race : on initialise callRef SYNCHRONIQUEMENT pour fermer la
      // fenêtre où call:initiated arrive avant que useEffect ait propagé le
      // setCall vers callRef.current. handleInitiated lit toujours via
      // setCall(prev => …) en complément, donc double garantie.
      const initial: ActiveCall = {
        status: "outgoing_ringing",
        conversationId,
        remoteUserId,
        direction: "outgoing",
        // callId / expiresAt seront récupérés via call:initiated.
      };
      callRef.current = initial;
      setCall(initial);
  ...
  const acceptCall = useCallback(async () => {
    ...
    if (!c.callId) {
      // Sans callId on ne peut pas accepter côté serveur — étape 3 traitera
      // les injections externes. On nettoie l'UI proprement.
      console.warn("[Call] acceptCall blocked : missing callId (external injection)");
      finishCall("declined");
      return;
    }
    ...
    try {
      ...
      emit("call:accept", { callId: c.callId });
    } catch (err) {
  ...
  /** Toggle speaker / earpiece (compat — utilisé quand BT pas dispo). */
  ...
  /**
   * Étape 5 — Force une route audio explicite (earpiece/speaker/bluetooth).
   * Si BT demandé sans device disponible, le natif retombe sur earpiece et
   * renvoie fallback=true ; on aligne l'état UI sur la route effective.
   */
  const setRoute = useCallback(async (route: NativeAudioRoute) => {
    const result = await nativeSetAudioRoute(route);
    setAudioRouteState(result.mode);
    setIsSpeakerOn(result.mode === "speaker");
  }, []);

  // Sync au démarrage d'un appel : peuple les routes disponibles côté natif.
  useEffect(() => {
    if (!call) return;
    const active = call.status === "outgoing_ringing"
      || call.status === "incoming_ringing"
      || call.status === "connecting"
      || call.status === "connected";
    if (!active) return;
    let cancelled = false;
    void getAudioRoutes().then((payload) => {
      if (cancelled) return;
      setAvailableAudioRoutes(payload.available);
    });
    return () => { cancelled = true; };
  }, [call?.status]);

  // Listener natif : BT plug/unplug pendant l'appel → MAJ availableRoutes + route
  // courante (si le BT disparaît, le natif retombe déjà sur earpiece).
  useEffect(() => {
    let unsub: (() => void) | null = null;
    let cancelled = false;
    void addAudioRouteListener((payload) => {
      setAvailableAudioRoutes(payload.available);
      setAudioRouteState(payload.current);
      setIsSpeakerOn(payload.current === "speaker");
    }).then((u) => {
      if (cancelled) { try { u(); } catch { /* ignore */ } return; }
      unsub = u;
    });
    return () => {
      cancelled = true;
   ...
   *  Étape 3 : `callId` est désormais OBLIGATOIRE pour toute injection
   *  externe. Sans callId, on no-op silencieusement (pas de sonnerie, pas
   *  d'UI). Cela ferme le trou des vieilles notifs/URLs/FCM qui ne portent
   *  pas l'identifiant serveur unique. Les flux socket call:incoming
   *  passent par leur propre handler ci-dessous et ne sont pas concernés.
   */
  const injectIncomingCall = useCallback((
    ...
    if (!extras?.callId) {
      // Source externe non vérifiée → on ignore.
      return;
    }
    ...
    // Sync ref AVANT setCall : les consommateurs (acceptCall lancé en
    // setTimeout(0/100ms) après injectIncomingCall) lisent callRef.current
    // sans attendre le commit React.
  ...
  useEffect(() => {
    ...
    const handleIncoming = (data: { callId: string; conversationId: string; callerId: string; callType: "audio" | "video"; expiresAt: number }) => {
      ...
      // Sync ref immédiat avant le commit React, pour fermer la fenêtre de
      // race où acceptCall/handleEnded pourraient lire callRef.current = null
      // entre l'arrivée du socket et le re-render qui peuple callRef.
      callRef.current = next;
      setCall(next);
    };

    /** Notre call:initiate a été enregistré côté serveur — on reçoit le callId.
     *  Utilise setCall(prev => …) (état React de vérité) plutôt que
     *  callRef.current : si l'event arrive avant la propagation du useEffect
     *  qui sync callRef, on lit l'état réel et non un null transitoire. */
    const handleInitiated = (data: { callId: string; conversationId: string; targetUserId: string; callType: "audio" | "video"; expiresAt: number }) => {
      setCall((prev) => {
        if (!prev || prev.direction !== "outgoing") return prev;
        if (prev.conversationId !== data.conversationId || prev.remoteUserId !== data.targetUserId) return prev;
        if (prev.callId) return prev;
        const next = { ...prev, callId: data.callId, expiresAt: data.expiresAt };
        callRef.current = next; // sync immédiat (handlers suivants peuvent en dépendre)
        return next;
      });
    };

    /** Our outgoing call was accepted */
    const handleAccepted = async (data: { callId: string; conversationId: string; accepterId: string }) => {
      const c = callRef.current;
      if (!c || c.direction !== "outgoing") return;
      // Filtre callId : ignore les events relatifs à un autre appel.
      if (c.callId && data.callId && c.callId !== data.callId) return;
    ...
    const handleRejected = (data: { callId: string; conversationId: string; rejecterId: string }) => {
      const c = callRef.current;
      if (!c) return;
      if (c.callId && data.callId && c.callId !== data.callId) return;
      finishCall("declined");
    ...
    const handleEnded = (data: { callId: string; conversationId: string; enderId: string }) => {
      const c = callRef.current;
      if (!c) return;
      if (c.callId && data.callId && c.callId !== data.callId) return;
    ...
    const handleNoAnswer = (data: { callId: string; conversationId: string }) => {
      const c = callRef.current;
      if (!c) return;
      if (c.callId && data.callId && c.callId !== data.callId) return;
      finishCall("unanswered");
    ...
    on("call:initiated" as any, handleInitiated as any);
    ...
    return () => {
      ...
      off("call:initiated" as any, handleInitiated as any);
  ...
  return {
    ...
    /** Speaker on */
    isSpeakerOn,
    /** Route audio active (earpiece/speaker/bluetooth) — étape 5 */
    audioRoute,
    /** Routes disponibles côté natif (peut inclure 'bluetooth' si casque connecté) — étape 5 */
    availableAudioRoutes,
    /** Forcer une route audio (earpiece/speaker/bluetooth) — étape 5 */
    setRoute,
    /** Start an outgoing audio call */
    startCall,
