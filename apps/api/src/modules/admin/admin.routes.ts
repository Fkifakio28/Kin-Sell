...
// ══════════════════════════════════════════════════════════════════════════════
// ADMIN — IA Studio Ads (créatifs)
// ══════════════════════════════════════════════════════════════════════════════

router.get("/ia-studio/creatives", asyncHandler(async (req: AuthenticatedRequest, res) => {
  const { page, limit, status, adType, audienceType } = req.query as Record<string, string>;
  const { listCreatives } = await import("../ads/ai-ad-creative.service.js");
  const data = await listCreatives({ page: Number(page) || 1, limit: Number(limit) || 20, status, adType, audienceType });
  res.json(data);
}));

router.get("/ia-studio/creatives/:id", asyncHandler(async (req: AuthenticatedRequest, res) => {
  const { getCreativeById } = await import("../ads/ai-ad-creative.service.js");
  const item = await getCreativeById(req.params.id);
  if (!item) throw new HttpError(404, "Creative not found");
  res.json(item);
}));

router.post("/ia-studio/generate", asyncHandler(async (req: AuthenticatedRequest, res) => {
  const body = z.object({
    adType: z.string(),
    ...
    customCtaTarget: z.string().optional(),
    customMediaUrl: z.string().optional(),
    variantGroup: z.string().optional(),
    variantLabel: z.string().optional(),
  }).parse(req.body);
  const { generateCreative } = await import("../ads/ai-ad-creative.service.js");
  const creative = await generateCreative({ ...body, userId: req.auth!.userId } as any);
  res.json(creative);
}));

router.post("/ia-studio/auto-generate", asyncHandler(async (req: AuthenticatedRequest, res) => {
  const { autoGenerateInternalAds } = await import("../ads/ai-ad-creative.service.js");
  const result = await autoGenerateInternalAds();
  res.json(result);
}));

router.patch("/ia-studio/creatives/:id", asyncHandler(async (req: AuthenticatedRequest, res) => {
  const { updateCreative } = await import("../ads/ai-ad-creative.service.js");
  const updated = await updateCreative(req.params.id, req.body);
  res.json(updated);
}));

router.patch("/ia-studio/creatives/:id/status", asyncHandler(async (req: AuthenticatedRequest, res) => {
  const { status } = z.object({ status: z.enum(["DRAFT", "READY", "PUBLISHED", "ARCHIVED"]) }).parse(req.body);
  const { updateCreativeStatus } = await import("../ads/ai-ad-creative.service.js");
  const updated = await updateCreativeStatus(req.params.id, status);
  res.json(updated);
}));

router.delete("/ia-studio/creatives/:id", asyncHandler(async (req: AuthenticatedRequest, res) => {
  const { deleteCreative } = await import("../ads/ai-ad-creative.service.js");
  await deleteCreative(req.params.id);
  res.json({ ok: true });
}));

// ══════════════════════════════════════════════════════════════════════════════
// ADMIN — IA Ads (campagnes & diffusion)
// ══════════════════════════════════════════════════════════════════════════════

router.get("/ia-ads/stats", asyncHandler(async (_req: AuthenticatedRequest, res) => {
  const { getCampaignStats } = await import("../ads/ai-ad-publishing.service.js");
  const stats = await getCampaignStats();
  res.json(stats);
}));

router.get("/ia-ads/campaigns", asyncHandler(async (req: AuthenticatedRequest, res) => {
  const { page, limit, active, objective } = req.query as Record<string, string>;
  const { listCampaigns } = await import("../ads/ai-ad-publishing.service.js");
...
router.post("/ia-ads/campaigns", asyncHandler(async (req: AuthenticatedRequest, res) => {
  const body = z.object({
    creativeId: z.string(),
    campaignName: z.string(),
    objective: z.string(),
    audienceRole: z.string().optional(),
    audienceConditions: z.record(z.unknown()).optional(),
  ...
  const { createCampaign } = await import("../ads/ai-ad-publishing.service.js");
  const campaign = await createCampaign({ ...body, userId: req.auth!.userId });
  res.json(campaign);
}));

router.post("/ia-ads/campaigns/publish/:creativeId", asyncHandler(async (req: AuthenticatedRequest, res) => {
  const { publishCreativeAsCampaign } = await import("../ads/ai-ad-publishing.service.js");
  const campaign = await publishCreativeAsCampaign(req.params.creativeId, req.body);
  res.json(campaign);
}));

router.patch("/ia-ads/campaigns/:id/toggle", asyncHandler(async (req: AuthenticatedRequest, res) => {
  const { active } = z.object({ active: z.boolean() }).parse(req.body);
  const { toggleCampaign } = await import("../ads/ai-ad-publishing.service.js");
  const updated = await toggleCampaign(req.params.id, active);
  res.json(updated);
}));

router.patch("/ia-ads/campaigns/:id", asyncHandler(async (req: AuthenticatedRequest, res) => {
  const { updateCampaign } = await import("../ads/ai-ad-publishing.service.js");
  const updated = await updateCampaign(req.params.id, req.body);
  res.json(updated);
}));

router.delete("/ia-ads/campaigns/:id", asyncHandler(async (req: AuthenticatedRequest, res) => {
  const { deleteCampaign } = await import("../ads/ai-ad-publishing.service.js");
  await deleteCampaign(req.params.id);
  res.json({ ok: true });
}));

router.get("/ia-ads/campaigns/:id/performance", asyncHandler(async (req: AuthenticatedRequest, res) => {
  const days = Number(req.query.days) || 30;
  const { getCampaignPerformance } = await import("../ads/ai-ad-placement.service.js");
  const perf = await getCampaignPerformance(req.params.id, days);
  res.json(perf);
}));

router.get("/ia-ads/top-campaigns", asyncHandler(async (_req: AuthenticatedRequest, res) => {
  const { getTopCampaigns } = await import("../ads/ai-ad-placement.service.js");
  const top = await getTopCampaigns(10);
  res.json(top);
}));
